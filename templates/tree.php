...
foreach ($page['entries'] as $e) {
	...
	if ($e['type'] === 'blob') {
		...
		echo "\t<td><a href=\"". makelink(array('a' => 'archive', 'p' => $page['project'], 'h' => $e['hash'], 't' => 'targz', 'n' => $e['name'])) ."\" class=\"tar_link\">tar.gz</a> / <a href=\"". makelink(array('a' => 'archive', 'p' => $page['project'], 'h' => $e['hash'], 't' => 'zip', 'n' => $e['name'])) ."\" class=\"zip_link\">zip</a></td>\n";
	}
	echo "</tr>\n";
}
?>
